    ...
    return (
      <div className="ListBranded">
        ...
        {this.props.branded.map((item, key) => {
          if (key <= 4) return <ItemBranded key={key} {...item} handleAddOnClick={this.handleAddOnClick} />;
          else return null;
        })}
        {this.props.branded.length === 0 && <div className="ListBrandedNoElement noselect">No items found.</div>}
      </div>
    );
  }
}
