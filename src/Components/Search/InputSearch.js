...
class InputSearch extends Component {
  handleOnKeyDown = (event) => {
    if (event.which === 27) this.props.dispatch(actionsGeneral.searchModalSet(false));
  };

  handleOnChange = (event) => {
    if (this.inputSearch.value === "") this.props.dispatch(actionsGeneral.searchModalSet(false, ""));
    else this.props.dispatch(actionsGeneral.searchModalSet(true, this.inputSearch.value));
  };

  handleOnClick = () => {
    if (this.inputSearch.value !== "") this.props.dispatch(actionsGeneral.searchModalSet(true, this.inputSearch.value));
  };
